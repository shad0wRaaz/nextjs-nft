...
const Nft = (props) => { //props are from getServerSideProps
  ...
  const {data: royaltyData, status: royaltyStatus} = useQuery(
    ...
    async() => {
      ...
      const contract = await sdk.getContract(nftContractData.token_address);
      const rr = await contract.royalties.getTokenRoyaltyInfo(
        nftContractData.token_id,
      );
      return rr
    },
    {
      enabled: Boolean(nftContractData?.token_address),
      onSuccess: (res) => {
        // console.log(res)
      }
    }
  ...
  useEffect(() => {
      ...
      ;(async () => {
        const nftImagePath = await getImagefromWeb3(nftContractData?.normalized_metadata?.image);
        const nftAVPath = await getImagefromWeb3(nftContractData?.normalized_metadata?.animation_url);
  ...
  useEffect(() => {
    if(!nftContractData || nftContractData === null || !Boolean(nftContractData?.normalized_metadata)) return

    if(Boolean(nftContractData.normalized_metadata?.animation_url)){
      const audioExts = ['m4a', 'wav', 'wma', 'aac', 'flac', 'mp3', 'ogg'];
      const videoExts = ['mov', 'flv', 'mp4', '3gp', 'mpg', 'wmv', 'webm', 'avi', 'mkv'];
      const nftExt = nftContractData.normalized_metadata?.animation_url?.slice(-3);
  ...
  return (
    <div className={` ${dark ? 'darkBackground text-neutral-100' : ' gradSky-vertical-white'}`}>
        ...
        <div className={`h-full rounded-md overflow-hidden fixed z-20 w-full top-0 left-0 backdrop-blur-3xl bg-[#00000022] flex justify-center items-center cursor-pointer ${ isZoom ? 'block' : 'hidden'}`}
         onClick={() => setIsZoom(false)}>
          <div className="relative">
            {!nftContractData ? (
              ...
              <ThirdwebNftMedia metadata={nftContractData.normalized_metadata} className="!h-full !w-full !object-cover rounded-md"/>
            }
      ...
      {(contractAddress === 'undefined' || nftContractData === null) ? (
        ...
        <>
          <SEO 
            title={nftContractData?.normalized_metadata?.name}
            description={nftContractData?.normalized_metadata?.description}
            image={imgPath}
            currentUrl={`https://nuvanft.io/nft/${thisNFTblockchain}/${contractAddress}/${nftContractData?.token_id}`} />
          {isBlocked ? (
              ...
              <div className="grid w-full grid-cols-1 gap-10 px-[1.2rem] md:gap-14 lg:grid-cols-2">
                <div className="space-y-8">
                  ...
                  >
                    <div className="aspect-w-11 aspect-h-12 overflow-hidden rounded-2xl max-h-[38rem] cursor-zoom-in" onClick={() => setIsZoom(true)}>
                      {!nftContractData ? (
                        ...
                        <ThirdwebNftMedia metadata={nftContractData.normalized_metadata} className="!h-full !w-full !object-cover"/>
                      }
                      {/* {playItem && itemType == "video" && (
                        <>
                          <video className="w-full h-full" autoPlay loop>
                  ...
                  <GeneralDetails
                    ...
                    ownerAddress={ownerData?.owner_of}
                      ...
                      <Tab.Panels className="mt-2">
                        <Tab.Panel className={'rounded-xl p-3'}>
                          <div className="flex flex-row justify-between py-2 flex-wrap break-words">
                            ...
                            <div className="flex gap-1 items-center">
                                <Link href={`/collection/${thisNFTblockchain}/${contractAddress}`} passHref>
                                  <a>
                                    <span className="line-clamp-1 text-sm hover:text-sky-600 transition">{nftContractData?.token_address}</span>
                                  </a>
                                </Link>
                                <Link href={`${chainExplorer[blockchainIdFromName[thisNFTblockchain]]}address/${nftContractData?.token_address}`} passHref>
                                  <a target="_blank">
                                    <MdOutlineOpenInNew />
                                  </a>
                                </Link>
                          ...
                          <div className="flex flex-row justify-between py-2 flex-wrap break-words">
                            ...
                            <span className="line-clamp-1 text-sm">{nftContractData?.token_id}</span>
                          </div>
                        ...
                        <Tab.Panel className={'rounded-xl text-sm ring-white p-3 ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2'}>
                          {nftContractData?.normalized_metadata?.description == '' ? 
                            <p className={`text-sm my-5 text-center ${dark ? 'text-slate-500' : 'text-neutral-600'}`}>
                              No description provided
                            </p> : nftContractData?.normalized_metadata?.description}
                        </Tab.Panel>
                        <Tab.Panel className={'rounded-xl p-3 my-5  ring-white ring-opacity-60 ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2'}>
                          <div className="flex flex-wrap gap-2">
                            {nftContractData?.normalized_metadata?.attributes?.map(
                              (props, id) => (
                                <div key={id}>
                                  {props.propertyKey != "" ? (
                                    <div
                            ...
                            {nftContractData?.normalized_metadata?.properties?.traits?.map(
                              (props, id) => (
                ...
                <div className={`border-t ${dark ? ' border-slate-600' : ' border-neutral-200'} pt-10 lg:border-t-0 lg:pt-0 xl:pl-10 relative`}>
                  <div className="sticky lg:top-[150px]">
                    <Purchase
                      ...
                      ownerAddress={ ownerData?.owner_of || null }
...
export async function getServerSideProps(context){
  ...
  const supportedChains = ["binance", "binance-testnet", "ethereum", "mainnet", "goerli", "polygon", "mumbai", "avalanche", "avalanche-fuji", "arbitrum"];
  ...
  if(chain === 'undefined' || contractAddress === 'undefined' || (!supportedChains.includes(String(chain).toLowerCase()))) {
    ...
    if(supportedChains.includes(chain)){
      ...
      // try{
        const endPoints = [
          `${HOST}/api/moralis/getNFTMetadata/${chain}/${contractAddress}/${tokenId}`,
          `${HOST}/api/moralis/getCollectionSanityData/${chain}/${contractAddress}/`,
          `${HOST}/api/moralis/getNFTOwnerData/${chain}/${contractAddress}/${tokenId}`,
          `${HOST}/api/mango/getSingle/${chain}/${contractAddress}/${tokenId}`,
        ]

        const unresolved = endPoints.map(point => fetch(point));
        const resolved = await Promise.all(unresolved);
        nftContractData = await Promise.resolve(resolved[0].json());
        sanityData = await Promise.resolve(resolved[1].json());
        ownerData = await Promise.resolve(resolved[2].json());
        nftData = await Promise.resolve(resolved[3].json());
        // const unresolvedJSON = resolved.map(data => data.json());
        // resolvedJSON = await Promise.all(unresolvedJSON);

        // nftContractData = resolvedJSON[0];

        // nftContractData = {
        //   ...nftContractData,
        //   metadata: JSON.parse(nftContractData.normalized_metadata)
        // }
    ...
    return {
      props: {
        contractAddress,
        listingData: nftData[0] || null, //listing data of the nft in the marketplace
        metaDataFromSanity: sanityData, //this is collection data
        nftContractData: { ...nftContractData }, //this is nft's contract data with metadata
