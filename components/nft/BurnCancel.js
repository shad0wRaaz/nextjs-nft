...
const BurnCancel = ({nftContractData, listingData, collectionAddress}) => {
    ...
    const collectionContract = useContract(collectionAddress, "nft-collection").contract;
// console.log(collectionContract)
    ...
    const burn = (e, sanityClient = config, toastHandler = toast, contract = collectionContract) => {
  ...
  return (
    ...
    {showModal && (
        <div className="fixed w-full h-full bg-slate-700 bg-opacity-75 top-0 left-0 z-30 backdrop-blur-sm flex align-items-center justify-center">
            <div className="relative w-[500px] rounded-xl py-[2em] px-[3em] bg-slate-50 m-auto">
                <p className="fw-700 text-base">Are you sure you want to burn this NFT?</p>
                <p className="text-xs text-gray-500">Burning does not mean deleting the NFT. It will only be made unclaimable by anyone. <span className="text-red-500">This action is irreverible!</span></p>
