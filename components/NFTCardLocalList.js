...
const NFTCardLocalList = ({ nftItem, listings }) => {
  ...
  return (
    <tr>
        <td className="mb-4">
            <Link href={`/nfts/${nftItem._id}`}>
                <div className="h-[50px] w-[50px] relative overflow-hidden m-3">
                    {!isListed && nftmetadata?.metadata?.image &&(
                        <Image
                        src={nftmetadata?.metadata.image}
                        className="h-96 w-96 relative rounded-2xl transition-transform duration-300 ease-in-out will-change-transform hover:scale-[1.03]"
                        objectFit="cover"
                        layout="fill"
...
export default NFTCardLocalList
